...
async function run() {
  ...
  for (const card of data.cards) {
    // Only drop expired rules. Keep not-yet-started (startsAt) rules for mid-month launches.
    card.benefits = (card.benefits || []).filter((benefit) => !isExpired(benefit.expiresAt));
    card.merchantRewards = (card.merchantRewards || []).filter((rule) => !isExpired(rule.expiresAt));
    card.planHintRules = (card.planHintRules || []).filter((rule) => !isExpired(rule.expiresAt));
    card.noRewardExceptionRules = (card.noRewardExceptionRules || []).filter(
      (rule) => !isExpired(rule.expiresAt)
    );
